...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Relatienummer'])) {
    $relatienummer = $_POST['Relatienummer'];

    // delete query
    $stmt = $pdo->prepare("DELETE FROM LedenOverzicht WHERE Relatienummer = :relatienummer");
    $stmt->bindParam(':relatienummer', $relatienummer, PDO::PARAM_STR);

    if ($stmt->execute()) {
        header("Location: ledenoverzicht.php?status=verwijderd");
        exit;
    } else {
